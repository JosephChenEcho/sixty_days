/* sixty_days — minimal visual novel engine.
   Script format (js/story.js, const STORY):
     {bg:'key'}                 crossfade background
     {music:'key'}              switch BGM (title/calm/tense/sad/end)
     {chapter:'第一章：七楼'}    full-screen chapter card
     {n:'...'}                  narration
     {say:['Name','...']}       dialogue
     {choice:[{t,do?,goto?}]}   branch; do = inline commands, goto = label
     {jump:'label'}
     {end:true}
*/
const BG = {
  title:  'assets/bg-title.jpg',
  office: 'assets/bg-office.jpg',
  cafe:   'assets/bg-cafe.jpg',
  car:    'assets/bg-car.jpg',
  beach:  'assets/bg-beach.jpg',
...
function runCmd(cmd) {
  ...
  if (cmd.jump) { S.label = cmd.jump; S.index = 0; next(); return; }
...
function showChoices(options) {
  ...
  options.forEach((opt) => {
    ...
    b.onclick = (e) => {
      ...
      if (opt.do) S.queue.unshift(...opt.do);
      if (opt.goto) { S.label = opt.goto; S.index = 0; }
      next();
    };
    box.appendChild(b);
  });
  box.classList.remove('hidden');
  autoSave(); // 选项出现的位置自动存档（含上面的 choice 修正）
}
...
function showEnd() {
  ...
  localStorage.removeItem('sixty_days_auto');
}

/* ---------- save / load ---------- */
function getSlots() {
  try { return JSON.parse(localStorage.getItem('sixty_days_slots')) || [null, null, null]; }
  catch (e) { return [null, null, null]; }
}
function setSlots(s) { localStorage.setItem('sixty_days_slots', JSON.stringify(s)); }
function snapshot() {
  // 选项界面存档时，index 指回选项命令本身，读档后选项重新出现
...
function autoSave() {
  ...
  try { localStorage.setItem('sixty_days_auto', JSON.stringify(snapshot())); } catch (e) {}
}
...
function renderSlots() {
  ...
  const box = $('slots');
  box.innerHTML = '';
  const auto = (() => { try { return JSON.parse(localStorage.getItem('sixty_days_auto')); } catch (e) { return null; } })();
...
preloadBG();
backToTitle();
// 首次用户手势时启动标题音乐（自动播放策略要求手势后才能播放音频）
